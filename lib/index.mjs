...
function apply(ctx) {
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			const method = pathname.startsWith("/session-search-toggle/api/") ? pathname.slice(27) : void 0;
			if (method === void 0 || method.includes("/")) {
				writeJson(res, 404, {
					ok: false,
					error: "unknown session-search-toggle API method"
				});
				return;
			}
			try {
				...
				if (method === "list-sessions") result = await listSessions(ctx);
					...
					writeJson(res, 404, {
						ok: false,
						error: `unknown session-search-toggle API method "${method}"`
					});
					return;
				}
